          ...
          <div className="relative">
            ...
            <div className="mt-8 border-t border-border pt-6">
              ...
              {completedDays.length === 0 ? (
                <div className="p-5 rounded-none bg-muted/10 border border-border/60 text-center text-xs text-muted-foreground/80">
                  {tx.aiCoachEmpty}
                </div>
              ) : (
                <div className="grid md:grid-cols-3 gap-6 items-stretch">
                  {/* Left Impact Statistics */}
                  <div className="md:col-span-1 p-4 rounded-none bg-muted/10 border border-border/85 flex flex-col justify-around gap-2 text-xs">
                    <div className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mb-2 border-b border-border/40 pb-2">
                      {tx.impactLabel}
                    </div>
                    {ecoCoachImpact.co2Saved > 0 && (
                      <div className="flex justify-between items-center py-1.5 border-b border-border/30">
                        <span className="text-muted-foreground">{tx.co2Label}</span>
                        <span className="font-semibold text-foreground">{ecoCoachImpact.co2Saved} kg</span>
                      </div>
                    )}
                    {ecoCoachImpact.waterSaved > 0 && (
                      <div className="flex justify-between items-center py-1.5 border-b border-border/30">
                        <span className="text-muted-foreground">{tx.waterLabel}</span>
                        <span className="font-semibold text-foreground">{ecoCoachImpact.waterSaved} L</span>
                      </div>
                    )}
                    {ecoCoachImpact.plasticSaved > 0 && (
                      <div className="flex justify-between items-center py-1.5 border-b border-border/30">
                        <span className="text-muted-foreground">{tx.plasticLabel}</span>
                        <span className="font-semibold text-foreground">{ecoCoachImpact.plasticSaved} kg</span>
                      </div>
                    )}
                    {ecoCoachImpact.communityCount > 0 && (
                      <div className="flex justify-between items-center py-1.5">
                        <span className="text-muted-foreground">{tx.communityActionLabel}</span>
                        <span className="font-semibold text-foreground">{ecoCoachImpact.communityCount}</span>
                      </div>
                    )}
                  </div>

                  {/* Right Coach Analysis Feedback */}
                  <div className="md:col-span-2 p-5 rounded-none bg-primary/[0.01] border border-primary/20 flex flex-col justify-between">
                    <div>
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-none border border-primary/20 text-[9px] font-mono tracking-widest font-bold text-primary mb-3 uppercase bg-primary/5">
                        {tx.aiCoachFeedback}
                      </div>
                      <p className="text-xs text-foreground leading-relaxed">
                        {completedDays.length === 7 ? (
                          lang === 'id' 
                            ? 'Anda telah menyelesaikan seluruh rangkaian Program Aksi Pelestarian. Tindakan nyata ini menunjukkan penerapan prinsip kearifan lokal dalam kehidupan sehari-hari—dari pengurangan jejak karbon hingga pengelolaan sumber daya yang bertanggung jawab. Sertifikat Kontributor Pelestarian telah tercatat dalam rekam jejak pembelajaran Anda.'
                            : 'You have completed the full Preservation Action Program. These real-world actions demonstrate the application of local wisdom principles in daily life—from reducing carbon footprint to responsible resource management. The Preservation Contributor certificate has been recorded in your learning journey.'
                        ) : (
                          lang === 'id'
                            ? `Berdasarkan ${completedDays.length} aksi yang telah diselesaikan, estimasi dampak lingkungan Anda mencakup reduksi emisi karbon, penghematan air, dan pengurangan sampah plastik. Setiap langkah kecil berkontribusi pada pelestarian ekosistem. Lanjutkan program untuk melengkapi sertifikat Kontributor Pelestarian.`
                            : `Based on ${completedDays.length} completed actions, your estimated environmental impact includes carbon emission reduction, water savings, and plastic waste reduction. Each small step contributes to ecosystem preservation. Continue the program to complete the Preservation Contributor certificate.`
                        )}
                      </p>
                    </div>
                  </div>
